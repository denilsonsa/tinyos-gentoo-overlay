...
src_compile() {
	...
	find net/tinyos/message -name "*.java" | xargs $(java-config -c) -source 1.4 -classpath ${cp} -nowarn || die "Failed to compile"
	einfo "Packaging TinyOS messages"
	find net/tinyos/message -name "*.class" | xargs jar cf ${PN}.jar
}

src_install() {
	java-pkg_dojar ${PN}.jar
	dodoc net/tinyos/message/README
