...
src_compile() {
	...
	einfo "Compiling TinyOS packet"
	find net/tinyos/packet -name "*.java" | xargs $(java-config -c) -source 1.4 -classpath ${cp} -nowarn || die "Failed to compile"
	einfo "Packaging TinyOS packet"
	find net/tinyos/packet -name "*.class" | xargs jar cf ${PN}.jar
}

src_install() {
	java-pkg_dojar ${PN}.jar
	use source && java-pkg_dosrc /net/tinyos/packet
}
