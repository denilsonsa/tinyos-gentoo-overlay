...
src_compile() {
	...
	find net/tinyos/util -name "*.java" | xargs $(java-config -c) -source 1.4 -classpath ${cp} -nowarn || die "Failed to compile"
	einfo "Packaging TinyOS java utils"
	find net/tinyos/util -name "*.class" | xargs $(java-config -j) cf ${PN}.jar
}
